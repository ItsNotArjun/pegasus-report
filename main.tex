...
\lfoot{\includegraphics[height=0.8cm]{logo_peggy.png}}
...
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash\bfseries\columncolor{db}}m{2.5cm}|>{\centering\arraybackslash}X|}
\hline
...
& Carry min. 3 passengers per declared cargo item during Tech Inspection \\ \arrayrulecolor{db}\cline{2-2}
...
From these calculations, the horizontal volume coefficient (HVC) was found to be \textbf{0.664\,m$^3$}, and the vertical volume coefficient (VVC) was determined as \textbf{0.071\,m$^3$}.
...
The fuselage sizing process was primarily driven by the size and structural requirements of the internal payloads. The cargo and passenger placement and size are decided based on mass distribution, as discussed in the static stability analysis in section ~\ref{sec:final_concept_design}. The heaviest payload- the cargo- was thus placed near the front to counterbalance the propulsion batteries and passengers, maintaining optimal CoG location. The cross-sectional area of 180 mm x 180 mm was decided to accommodate the width of the passenger compartment and the banner compartment under the mid-wing. The 995 mm length is established to accommodate all the compartments comfortably; all the compartment placements are shown in \hyperref[fig:fuselage_top]{Figure~\ref*{fig:fuselage_top}}. The nose and the end of the fuselage are tapered to reduce drag without compromising strength.
...
\begin{center}
    ...
    \captionof{figure}{Root Locus for Longitudinal Stability (Left) and Lateral Stability (Right)}
    \label{fig:dynamic}
\end{center}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{wiring2.png}
...
\subsubsubsection{Passenger Module Overview and Material Selection}
...
Each passenger location includes an individual horseshoe-shaped clip mounted on a transverse support rod. The rod itself is fixed to the structure and is also free to rotate independently about its own. The clips are manufactured from Acrylonitrile butadiene styrene (ABS), which is used because it provides enough ductility to elastically deform and snap securely around the passenger while maintaining strength and durability over repeated use.
During loading, a clip is rotated upward about its pivot to create clearance. The passenger is then positioned and pressed into the horseshoe feature, after which the clip is rotated back down to its original orientation. Once returned to this position, the TPU's elastic deformation generates a continuous clamping force around the duck's neck, restraining it against vertical, lateral, and longitudinal motion. Because each clip operates independently, passengers can be installed one at a time without interference from adjacent rows, allowing straightforward sequential loading.


This approach keeps the mechanism mechanically simple while still providing positive retention. The open layout also accommodates small variations in passenger size without requiring tight tolerances. Loads from the restrained passengers are transmitted from the TPU clips into the support rod and then into the Depron base, which distributes them into the fuselage through a direct load path appropriate for the relatively small payload mass.


Access to the compartment is provided through a hinged door located above the passenger bay. The door remains attached to the aircraft through its hinge in both open and closed positions. A spring-loaded piston latch secures the door during flight; actuating an external lever releases the latch for opening, and releasing it re-engages the lock. This provides a positive mechanical closure while allowing quick access during loading and unloading.
...
A separate 3S 1500 mAh LiPo battery is used to power all the other avionics on board the plane via an avionics BEC. An 8-channel receiver was used, which facilitates the control of all actuators on the plane.
...
\hline
\textcolor{gray!5}{Motor 1} & 0.700 kg &0.700 kg& 0.375 m on the right side of the wing from the wing root \\ 
\hline
\textcolor{gray!5}{Motor 2} & 0.700 kg & 0.700 kg &0.375 m on the left side of the wing from the wing root  \\ 
\hline
...
\hline
\textcolor{gray!5}{Landing Gear (Left Wheel)} & 0.175 kg & 0.102 kg& 0.10 m behind the leading edge, 0.20 m from the wing root, 0.20 m below the leading edge \\ 
\hline
\textcolor{gray!5}{Landing Gear (Right Wheel)} & 0.175 kg & 0.102 kg & 0.10 m behind the leading edge, 0.20 m from the wing root, 0.20 m below the leading edge \\ 
\hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm,height=7.5cm]{manu - Sheet1 (1)_cropped.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm,height=6cm]{test - Sheet1_cropped.pdf}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Main Wing CFRP Test} % Changed from \captionof to \caption inside a figure environment
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Empennage square CFRP rod} % Changed from \captionof to \caption
...
To fulfill the competition's structural verification requirements, a wingtip bending test was conducted to evaluate the main wing's load-bearing capacity.  The aircraft was fully configured to 1.5 times its MTOW (12 kg), which included the maximum declared payload and the heaviest battery packs. The aircraft was then lifted by a single support point at each wingtip, simulating a roughly 1.5g flight load case. Dynamic loads were introduced by applying brief downward forces to the fuselage to verify the dynamic structural capabilities of the aircraft. The wings successfully supported the entire weight without exhibiting structural failure or permanent deformation. Additionally, this procedure allowed the team to verify that the loaded center of gravity (CG) remained properly aligned with the required exterior markings.
...
Our analysis indicates that the maximum wind speed we will encounter during a flight session is [80 km/h]. Adding our cruise speed of [Cruise Speed] to this results in a maximum relative wind speed of [Total Speed] at which our banner has to deploy. We tested our mechanism at 3 different speeds in both turbulent and non-turbulent airflow. Our banner was deployed successfully in all configurations. The maximum stress faced during the test was [Calculated Stress], while external testing shows that our banner mechanism can handle a yield stress of [Material Yield Stress]. This provides a factor of safety of [Calculated FOS].
...
Rubber Duck Compartment & $\square$ \\ \hline
Hockey Puck Compartment & $\square$ \\ \hline
...
\cellcolor{db}\color{gray!5}Weight & 7.2 kg & 6.9 kg & -4.1\\ \hline
...
The propulsion system's parameters were measured from the ESC data log and using a load-cell so as to assess the flight performance of propulsion system component and practically access the performance of the motors. Each propulsion pack showed considerable deviations of multiple parameters, with peak current exceeding the design point, rpm being offset by nearly 500 rpm and estimated thrust deviating of 10 N from the theoretically calculated values.

% --- SECTION 8.1: PROTOTYPE-1 (6.9 kg) ---
\section{Propulsion and Power Analysis (Prototype-1: 6.9 kg)}

\noindent
\begin{minipage}[t]{0.60\textwidth}
    \vspace{0pt} 
    The results of the initial propulsion thrust tests for Prototype-1 are shown in Figure 8.1. As illustrated, the motor setup produced a maximum static thrust of 65.0 N (approximately 6.6 kg). Theoretical predictions for in-flight performance suggested a healthy dynamic thrust of 50.0 N. Given the initial design weight of 6.9 kg (67.6 N), this configuration provided a robust thrust-to-weight ratio, ensuring sufficient authority for takeoff and climb phases.

    \medskip
    Power consumption correlated well with expected brushless motor curves. It should be noted that a 15\% drop in electrical power was predicted during flight due to propeller unloading, as the motor was able to "rev up" by approximately 8\% compared to static ground tests. This increased efficiency validated the 6.9 kg configuration as the ideal design point.
\end{minipage}
\hfill
\begin{minipage}[t]{0.38\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=\textwidth]{power_p1_final.png}
    \captionof{figure}{Power (W) - P1}
    \label{fig:p1_power}
\end{minipage}
...
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{rpm_p1_final.png}
    \captionof{figure}{Motor RPM - P1}
    \label{fig:p1_rpm}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{thrust_p1_newtons.png}
    \captionof{figure}{Thrust (N) - P1}
    \label{fig:p1_thrust}
\end{minipage}

\vspace{3em}

% --- SECTION 8.2: PROTOTYPE-2 (7.5 kg) ---
\section{Propulsion and Power Analysis (Prototype-2: 7.5 kg)}

\noindent
\begin{minipage}[t]{0.60\textwidth}
    \vspace{0pt}
    The performance characteristics for Prototype-2, following the 0.6 kg weight increase, are shown below. While the static thrust remained consistent, the total aircraft weight increased to 73.5 N. This resulted in a significantly narrowed safety margin, as the dynamic thrust was inferred to drop to 48.0 N due to the higher induced drag required to maintain lift at the increased mass.

    \medskip
    The "unloading" effect was suppressed in this configuration, with power consumption only dropping to 91\% of static levels. This behavior, alongside a stunted RPM increase of only 4\%, indicates the motor was operating under excessive load. Whilst the structural integrity of the Aeroply fuselage was maintained, the propulsion system was unable to overcome these drag penalties.
\end{minipage}
\hfill
\begin{minipage}[t]{0.38\textwidth}
    \vspace{0pt}
    \centering
    \includegraphics[width=\textwidth]{power_p2_final.png}
    \captionof{figure}{Power (W) - P2}
    \label{fig:p2_power}
\end{minipage}
...
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{rpm_p2_final.png}
    \captionof{figure}{Motor RPM - P2}
    \label{fig:p2_rpm}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{thrust_p2_newtons.png}
    \captionof{figure}{Thrust (N) - P2}
    \label{fig:p2_thrust}
\end{minipage}
